...
export const monthlyCharges = sqliteTable("monthly_charges", {
  id: text("id").primaryKey(), billingMonth: text("billing_month").notNull(), amountBilled: real("amount_billed").notNull(), dueDate: text("due_date").notNull(), isPaid: integer("is_paid", { mode: "boolean" }).notNull().default(false), contractId: text("contract_id").notNull().references(() => ownershipContracts.id), ...timestamps,
