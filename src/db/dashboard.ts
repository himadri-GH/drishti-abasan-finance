...
export async function getUnitDirectory() {
  if (!db) return [];
  return db.select({ contractId: ownershipContracts.id, ownerId: owners.id, unitId: propertyUnits.id, unitCode: propertyUnits.unitCode, unitType: propertyUnits.unitType, block: propertyUnits.block, floorNumber: propertyUnits.floorNumber, ownerName: owners.fullName, phone: owners.phone,email: owners.email, status: ownershipContracts.status, monthlyRate: ownershipContracts.monthlyRate, balance: sql<number>`${ownershipContracts.openingBalance} + coalesce((select sum(mc.amount_billed) from monthly_charges mc where mc.contract_id = ${ownershipContracts.id}), 0) - coalesce((select sum(pa.amount_applied) from payment_allocations pa where pa.charge_id in (select mc2.id from monthly_charges mc2 where mc2.contract_id = ${ownershipContracts.id}) and pa.status = 'ACTIVE'), 0)` }).from(ownershipContracts)
    .innerJoin(propertyUnits, eq(ownershipContracts.propertyUnitId, propertyUnits.id))
