              ...
              ) 
{
  ...
  const start = (contract?: Contract) => { setEditing(contract ?? null); setMessage(""); setOpen(true); };
  return <><button className={styles.primaryButton} onClick={() => start()}>＋ New contract</button>{open && <div className={styles.modalBackdrop}><form className={styles.modal} onSubmit={save}><div className={styles.modalHeading}><div><p className={styles.eyebrow}>Ownership contract</p><h3>{editing ? "Edit contract" : "Create contract"}</h3></div><button type="button" className={styles.closeButton} onClick={() => setOpen(false)}>×</button></div>{editing ? <><label>Owner and unit<input value={`${editing.ownerName} · ${editing.unitCode}`} readOnly /></label></> : <><label>Owner<select name="ownerId" required><option value="">Choose owner</option>{owners.map((item) => <option value={item.id} key={item.id}>{item.label}</option>)}</select></label><label>Property unit<select name="propertyUnitId" required><option value="">Choose unit</option>{units.map((item) => <option value={item.id} key={item.id}>{item.label}</option>)}</select></label><label>Contract code<input name="contractCode" placeholder="CTR-A101-2026" required /></label></>}<div className={styles.formRow}><label>Start date<input name="startDate" type="date" defaultValue={editing?.startDate ?? "2026-08-01"} required /></label><label>End date<input name="endDate" type="date" defaultValue={editing?.endDate ?? ""} /></label></div><div className={styles.formRow}><label>Monthly rate<input name="monthlyRate" type="number" min="1" defaultValue={editing?.rate ?? ""} required /></label><label>Opening balance<input name="openingBalance" type="number" defaultValue={editing?.opening ?? 0} /></label></div><label>Occupancy<select name="occupancyType" defaultValue={editing?.occupancy ?? "SELF_OCCUPIED"}><option>SELF_OCCUPIED</option><option>TENANT</option><option>VACANT</option></select></label>{editing && <label>Status<select name="status" defaultValue={editing.status}><option>ACTIVE</option><option>FROZEN</option><option>TRANSFERRED</option><option>VACATED</option></select></label>}{message && <p className={styles.formMessage}>{message}</p>}
