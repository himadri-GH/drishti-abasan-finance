...
export function UnitEditForm({ unit }: { unit: Unit }) {
  ...
  const modal = open && <div className={styles.modalBackdrop} role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setOpen(false)}><form className={styles.modal} onSubmit={submit}>
      ...
      <div className={styles.formRow}>
        ...
        <label>Unit type<select name="unitType" defaultValue={unit.unitType}><option>FLAT</option><option>GARAGE</option><option>SHOP</option><option>STORAGE</option></select></label></div>
